  ...
  return (
    <header className="topbar h-16 flex items-center justify-between px-8 sticky top-0 z-20">
      ...
      <div className="flex items-center gap-2 text-sm">
        {breadcrumb.map((crumb, i) => (
          <span key={i} className="flex items-center gap-2">
            ...
            <span className={`font-semibold ${i === breadcrumb.length - 1 ? 'text-gray-800' : 'text-gray-400'}`}>
              {crumb.label}
            </span>
          </span>
        ))}
      ...
      <div className="flex items-center gap-3">
        {/* Search */}
        <div className="hidden sm:flex items-center gap-2 bg-gray-100 hover:bg-gray-200 transition-colors px-3 py-2 rounded-xl text-sm text-gray-400 cursor-pointer">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            <circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>
          </svg>
          <span className="text-xs">Cari...</span>
          <kbd className="ml-1 text-[10px] bg-white border border-gray-200 px-1.5 py-0.5 rounded font-mono">⌘K</kbd>
        </div>
            ...
            {notifOpen && (
              ...
              >
                ...
                {notifications.map((n, i) => (
                  <div key={i} className={`px-4 py-3 flex gap-3 hover:bg-gray-50 transition-colors cursor-pointer ${n.unread ? '' : 'opacity-60'}`}>
        ...
        <div className="flex items-center gap-2.5 pl-2 border-l border-gray-200">
          ...
          <div className="hidden sm:block">
            <p className="text-sm font-bold text-gray-900 leading-tight">User</p>
